...
export const generateStudentIdCard = async (
  ...
  if (process.env.SCHOOL_LOGO) {
    try {
      const response = await fetch(process.env.SCHOOL_LOGO);
  ...
  doc
    ...
    .fillColor("black")
    .text(
      "Calabar Post 1, Block 1 Unit Enebong (Federal Housing Estate)",
      centerX + 50,
      frontY + 33
    )
    .text(
  ...
  const qrDataUrl = await QRCode.toDataURL(
    `${process.env.PUBLIC_DOMAIN}/admin/students/${student.id}`
  );
  ...
  doc
    ...
    .fontSize(7)
    .text(
      "If found, please return to the school address on the front.",
      centerX,
      backY + 145,
      {
        align: "center",
